            ...
            <div class="flex flex-col md:flex-row md:items-end justify-between gap-4">
                <div>
                    <nav class="flex items-center gap-2 text-[0.75rem] text-outline-variant mb-1 uppercase tracking-widest font-semibold">
                        ...
                        <span class="text-primary">登入日誌</span>
                    </nav>
                    <h2 class="text-[1.5rem] font-bold text-on-surface tracking-tight font-headline">登入日誌</h2>
                    <p class="text-[0.8125rem] text-outline mt-1">追蹤所有使用者帳號的登入紀錄與結果</p>
                </div>
            </div>
            ...
            <div class="bg-surface-container-lowest rounded-xl shadow-[0_24px_40px_-4px_rgba(23,28,31,0.06)] p-6">
                <form method="GET" action="{{ asset('admin/admin.login-log/list') }}">
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div class="space-y-2">
                            <label class="text-[0.75rem] font-bold text-outline uppercase tracking-wider">操作者名稱 / 帳號</label>
                            <input type="text"
                        ...
                        <div class="space-y-2">
                            <label class="text-[0.75rem] font-bold text-outline uppercase tracking-wider">操作</label>
                            <select name="action"
                        ...
                        <div class="space-y-2">
                            <label class="text-[0.75rem] font-bold text-outline uppercase tracking-wider">狀態</label>
                            <select name="status"
                                    class="w-full bg-surface-container-low border border-outline-variant/30 rounded-lg px-4 py-2.5 text-[0.875rem] text-on-surface focus:border-primary focus:ring-primary/30 focus:ring-2 transition-all">
                                <option value="">全部</option>
                    ...
                    <div class="flex justify-end gap-3 mt-8 pt-6 border-t border-outline-variant/20">
                        <a href="{{ asset('admin/admin.login-log/list') }}" class="px-6 py-2.5 text-outline hover:text-on-surface font-medium text-[0.875rem] transition-colors">
                            清除條件
                        </a>
                        ...
                        <table class="w-full border-collapse" id="logTable">
                            ...
                            @foreach ($data as $key => $row)
                                <tr class="hover:bg-surface-container-low transition-colors duration-200">
                                    ...
                                    <td class="px-6 py-5">
                                        <a href="{{ asset('admin/admin.login-log/detail/' . $row['id']) }}" class="flex items-center gap-1.5 text-primary hover:bg-primary/5 px-3 py-1.5 rounded-lg transition-colors font-semibold text-[0.875rem] no-underline">
                                    ...
                                    <td class="px-6 py-5 text-[0.875rem] text-on-surface">{{ $row['employee_name'] }}</td>
