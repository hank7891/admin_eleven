...
@section('title-suffix', ' · 登入日誌')

@section('content')
    <x-admin.page-head
        title="登入日誌"
        subtitle="追蹤所有使用者帳號的登入紀錄與結果"
        :breadcrumbs="[['label' => '首頁', 'url' => 'admin/'], ['label' => '登入日誌']]"
    />

    <x-admin.filter-card :action="asset('admin/admin.login-log/list')" title="搜尋條件">
        <x-admin.input
        ...
        <x-slot:actions>
            <a href="{{ asset('admin/admin.login-log/list') }}" class="admin-btn admin-btn-outline">清除條件</a>
                    ...
                    <tbody>
                        @foreach ($data as $key => $row)
                            <tr>
                                ...
                                <td>
                                    <a href="{{ asset('admin/admin.login-log/detail/' . $row['id']) }}" class="admin-link-action">
