...
@section('title-suffix', ' · 操作日誌詳情')

@section('content')
    <x-admin.page-head
        title="操作日誌詳情"
        subtitle="查看系統操作紀錄的完整細節"
        :breadcrumbs="[
            ['label' => '首頁', 'url' => 'admin/'],
            ['label' => '操作日誌', 'url' => 'admin/admin.log/list'],
            ['label' => '詳情'],
        ]"
    >
        <x-slot:actions>
            <a href="{{ asset('admin/admin.log/list') }}" class="admin-btn admin-btn-outline">
    ...
    <x-admin.card title="修改詳情">
                                ...
                                <td>
                                    <span class="admin-diff-pill admin-diff-pill-new">
                                        @if (is_array($change) && isset($change['new']))
                                            {{ is_array($change['new']) ? json_encode($change['new'], JSON_UNESCAPED_UNICODE) : ($change['new'] ?? '--') }}
                                        @else
                                            {{ is_array($change) ? json_encode($change, JSON_UNESCAPED_UNICODE) : ($change ?? '--') }}
                                        @endif
                                    </span>
        ...
        @else
            <p class="admin-help admin-flex admin-gap-sm">
