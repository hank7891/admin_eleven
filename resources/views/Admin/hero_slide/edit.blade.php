        ...
        <div class="p-6 lg:p-10 space-y-8">
            <div class="flex flex-col md:flex-row md:items-end justify-between gap-4">
                <div>
                    ...
                    <p class="text-[0.8125rem] text-outline mt-1">維護首頁主視覺輪播圖片、文案與點擊連結設定</p>
                </div>
            </div>
                ...
                <form id="hero-slide-edit-form" action="{{ asset('admin/hero-slide/edit') }}" method="POST" enctype="multipart/form-data" class="w-full lg:w-2/3 space-y-6">
                                ...
                                <label for="image" data-upload-dropzone class="block rounded-xl border border-dashed border-outline-variant/40 bg-surface-container-low px-6 py-6 cursor-pointer hover:border-primary/50 transition-colors">
                                    <div class="flex flex-col items-center justify-center gap-3 text-center">
                                        ...
                                        <div>
                                            ...
                                            <p class="mt-1 text-[0.75rem] text-outline-variant">建議尺寸 1920x1080（16:9 橫式），支援 jpg / jpeg / png / gif / webp，最大 {{ round((config('upload.image.max_size', 5120)) / 1024, 2) }}MB</p>
                                        </div>
                                    </div>
                                </label>
                                <input id="image" type="file" name="image" accept="image/*" class="hidden">
                                <div class="rounded-xl bg-surface-container-low p-4">
                                    <div class="mx-auto w-full max-w-xl overflow-hidden rounded-lg bg-surface-container-high aspect-[16/9]">
                                        <img id="imagePreview" src="{{ $data['image_url'] ?? '' }}" alt="輪播預覽" class="{{ empty($data['image_url']) ? 'hidden' : 'flex' }} h-full w-full object-cover">
                                        <div id="imagePlaceholder" class="{{ empty($data['image_url']) ? 'flex' : 'hidden' }} h-full w-full items-center justify-center text-outline-variant">
                                            <div class="text-center">
                                                <span class="material-symbols-outlined text-[32px]">image</span>
                                                <p class="mt-2 text-[0.75rem]">16:9 預覽比例</p>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                    ...
                    <div class="bg-surface-container-lowest rounded-xl shadow-[0_24px_40px_-4px_rgba(23,28,31,0.06)] overflow-hidden border-t-4 border-t-secondary">
                        ...
                        <div class="p-8 space-y-6">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div class="space-y-1.5 md:col-span-2">
                                    <label class="block text-[0.875rem] font-medium text-on-surface-variant">輪播點擊連結</label>
                                    <input type="url" name="target_url" value="{{ $data['target_url'] ?? '' }}" placeholder="https://example.com 或 /product" class="w-full bg-surface-container-high border border-transparent rounded-lg px-4 py-2.5 text-[0.875rem] text-on-surface focus:bg-white focus:border-primary focus:ring-2 focus:ring-primary/10 transition-all">
                                    <p class="text-[0.75rem] text-outline-variant">前台點擊輪播圖片將導向此連結，留空則不啟用點擊導頁</p>
                                </div>
                                <div class="space-y-1.5">
                                ...
                                <div class="space-y-1.5">
                                    ...
                                    @if ($isCreate)
                                        ...
                                        <select class="w-full bg-surface-container-high border border-transparent rounded-lg px-4 py-2.5 text-[0.875rem] text-outline" disabled>
                                            <option>{{ config('constants.status.' . STATUS_INACTIVE) }}</option>
                                        </select>
                                        <p class="text-[0.75rem] text-outline-variant">新增後才可於編輯模式切換為啟用</p>
                                    @else
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            const showPreview = (file) => {
                ...
                reader.onload = function (loadEvent) {
                    ...
                    imagePlaceholder.classList.replace('flex', 'hidden');
                };
            ...
            dropzone.addEventListener('drop', function (event) {
                ...
                if (!file || !file.type.startsWith('image/')) {
                    dropzone.classList.add('border-error', 'bg-error/5');
                    setTimeout(() => dropzone.classList.remove('border-error', 'bg-error/5'), 1500);
                    return;
                }
