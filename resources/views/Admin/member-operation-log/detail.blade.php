            ...
            <div class="flex flex-col md:flex-row md:items-end justify-between gap-4">
                <div>
                    <nav class="flex items-center gap-2 text-[0.75rem] text-outline-variant mb-1 uppercase tracking-widest font-semibold">
                        ...
                        <a href="{{ asset('admin/member.operation-log/list') }}" class="hover:text-primary transition-colors">會員操作日誌</a>
                        <span class="material-symbols-outlined text-[14px]">chevron_right</span>
                        <span class="text-primary">詳情</span>
                    </nav>
                    <h2 class="text-[1.5rem] font-bold text-on-surface tracking-tight font-headline">會員操作日誌詳情</h2>
                    <p class="text-[0.8125rem] text-outline mt-1">查看會員操作紀錄的完整細節</p>
                </div>
            </div>
            ...
            <div class="flex flex-col lg:flex-row gap-6">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-y-4 gap-x-8">
                                ...
                                @if($data['member_id'])
                                    <div class="flex items-center justify-between">
                                        ...
                                        <div class="bg-surface-container-low px-4 py-2 rounded-lg min-w-[200px] text-right font-mono text-[0.875rem] text-on-surface-variant">{{ $data['member_id'] }}</div>
                                    </div>
                                @endif
                                <div class="flex items-center justify-between">
                                            ...
                                            <tr class="hover:bg-surface-container-low transition-colors duration-200">
                                                ...
                                                <td class="px-6 py-5">
                                                    @if(is_array($change) && array_key_exists('old', $change))
                                                        <span class="bg-red-50 text-red-600 px-3 py-1 rounded-md text-[0.8125rem] font-medium break-all">{{ is_array($change['old']) ? json_encode($change['old'], JSON_UNESCAPED_UNICODE) : (is_null($change['old']) ? '--' : $change['old']) }}</span>
                                                    @else
                                                        <span class="bg-red-50 text-red-600 px-3 py-1 rounded-md text-[0.8125rem] font-medium break-all">{{ is_array($change) ? json_encode($change, JSON_UNESCAPED_UNICODE) : ($change ?? '--') }}</span>
                                                    @endif
                                                </td>
                                                <td class="px-6 py-5">
                                                    @if(is_array($change) && array_key_exists('new', $change))
                                                        <span class="bg-emerald-50 text-emerald-600 px-3 py-1 rounded-md text-[0.8125rem] font-medium break-all">{{ is_array($change['new']) ? json_encode($change['new'], JSON_UNESCAPED_UNICODE) : (is_null($change['new']) ? '--' : $change['new']) }}</span>
                                                    @else
                                                        <span class="bg-emerald-50 text-emerald-600 px-3 py-1 rounded-md text-[0.8125rem] font-medium break-all">{{ is_array($change) ? json_encode($change, JSON_UNESCAPED_UNICODE) : ($change ?? '--') }}</span>
                                                    @endif
                                                </td>
                                            </tr>
                                        @endforeach
                ...
                <div class="w-full lg:w-1/3">
                    <div class="bg-surface-container-lowest rounded-xl shadow-[0_24px_40px_-4px_rgba(23,28,31,0.06)] overflow-hidden sticky top-4">
                        ...
                        <div class="p-6">
                            <a href="{{ asset('admin/member.operation-log/list') }}" class="w-full py-3 btn-primary rounded-xl font-bold text-[0.875rem] flex items-center justify-center gap-2 hover:scale-[1.02] active:scale-95 transition-all duration-200 no-underline">
